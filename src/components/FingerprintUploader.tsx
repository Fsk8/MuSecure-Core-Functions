...
export function FingerprintUploader() {
    ...
    try {
      ...
      try {
        const out = await runCatalogAuthenticityCheck(file, acoustIdKey, (s) => {
          ...
          if (s === "acoustid")
            setProgress({ stage: "Consultando AcoustID / MusicBrainz…", percent: 80 });
        });
  ...
  return (
    <div className="card">
      ...
      <p className="muted">
        1) Genera huella (MuSecure + AcoustID/MusicBrainz) 2) Conecta wallet 3) Sube a IPFS
      </p>
      ...
      {result && (
        <section className="section">
          ...
          <dl className="dl">
            ...
            <dt>SHA-256</dt>
            <dd>
              <code className="mono">0x{result.sha256}</code>
            </dd>
          </dl>
        </section>
      )}
