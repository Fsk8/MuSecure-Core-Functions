 ...
 * MuSecure – Explorer
 *
 * - audioUrl() para evitar descargas forzadas
 * - pointer-events-none cuando CID no está disponible
 * - Sin href="#" — links vacíos usan undefined
 * - signMessage desde useWallet() (compatible con cualquier wallet)
 */
  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        ...
        const allFiles = await lh.listUploads();
  ...
  if (songs.length === 0) {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center gap-4 py-24">
        ...
        <p className="font-mono text-xs uppercase tracking-wider text-zinc-600">No hay obras publicadas todavía</p>
      </motion.div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 pt-2">
      {songs.map((song, index) => {
        ...
        // URL con ?filename=audio.mp3 — el browser reproduce en vez de descargar
        const publicAudioUrl = song.cid
          ? LighthouseService.audioUrl(song.cid, song.mimeType)
          : "";
            ...
            <Card className="group flex h-full flex-col hover:border-emerald-500/20 hover:shadow-lg hover:shadow-emerald-500/5">
              <div className="mb-6 flex items-center justify-between">
                <Badge variant={isEncrypted ? "default" : "secondary"}>
                  {isEncrypted
                    ? <><Lock className="mr-1 h-2.5 w-2.5" />Protegido</>
                ...
                {isEncrypted ? (
                  authenticated && address && signMessage ? (
                    ...
                    <Button onClick={() => login()} className="w-full" size="lg">
                      <Lock className="h-3.5 w-3.5" />Conectar para Escuchar
                    </Button>
                  )
                ) : (
                  <a
                    ...
                    className={[
                      ...
                      !publicAudioUrl
                        ...
                        : "hover:border-emerald-500/40 hover:bg-emerald-500/10 hover:shadow-lg hover:shadow-emerald-500/5",
                    ].join(" ")}
                  >
